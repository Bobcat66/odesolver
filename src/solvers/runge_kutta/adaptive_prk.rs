...
pub struct FirstOrderAdaptiveRKController<Method, const P: usize, const S: usize> 
    where Method: PRKMethod<P,S,1>
{
    _marker: PhantomData<Method>
}

impl<Method,const P: usize, const S: usize> FirstOrderAdaptiveRKController<Method, P, S>
    where Method: PRKMethod<P,S,1>
{
    const ERROR_EXPONENT: f64 = {-1.0/(Method::ERR_ORDER as f64 + 1.0)};
}

impl<Method, const P: usize, const S: usize> PRKController<P,1> for FirstOrderAdaptiveRKController<Method,P,S>
    where Method: PRKMethod<P,S,1>
{

    type Config = AdaptiveRKConfig;

    fn get_next_step<const D: usize>(y1: &SMatrix<f64,D,P>, e: &[SMatrix<f64,D,P>; 1], y0: &SMatrix<f64,D,P>, h: f64, cfg: &AdaptiveRKConfig) -> (bool, f64)
    {
        let err = e[0];
...
pub struct PRKShampineInterpolant<const P: usize, const S: usize, const K: usize, const D: usize>
{
    t0: f64,
    t1: f64,
    h: f64,
...
impl<const P: usize,const S: usize, const K: usize, const D: usize> PRKShampineInterpolant<P,S,K,D> 
{
...
impl<const P: usize,const S: usize, const K: usize, const D: usize> DenseInterpolant<P,D> for PRKShampineInterpolant<P,S,K,D>
{
...
pub struct ShampineRKInterpolator<Shampine,const P: usize, const K: usize, const S: usize>
    where Shampine: ShampineConfig<K,S>
{
    _marker: PhantomData<Shampine>
}

impl<Shampine, const P: usize, const K: usize, const S: usize> PRKInterpolator<P,S> for ShampineRKInterpolator<Shampine,P, K, S>
    where Shampine: ShampineConfig<K,S>
{
    type InterpolantType<const D: usize> = PRKShampineInterpolant<P,S,K,D>;
    fn interpolate_stage<F,const D: usize>(ode: &F, t0: f64, t1: f64, y0: &SMatrix<f64,D,P>, y1: &SMatrix<f64,D,P>, stage: &[SMatrix<f64,D,P>; S]) -> Self::InterpolantType<D>
        where F: Fn(f64,&SMatrix<f64,D,P>) -> SMatrix<f64,D,P>
    {
        PRKShampineInterpolant::<P,S,K,D>::new(t0, t1, *y0, *y1,*stage, Shampine::W)
    }
}
